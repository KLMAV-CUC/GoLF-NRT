...
def img2mse(x, y, mask=None):
    ...
    if mask is None:
        ...
        return torch.mean((x - y) * (x - y)* mask.unsqueeze(-1))
...
def get_ssim(pred_img, gt_img):
    ...
    # pred_rgb_converted = img_as_ubyte(pred_img)
    # gt_img_converted = img_as_ubyte(gt_img)
    # ssim = structural_similarity(pred_rgb_converted, gt_img_converted, channel_axis=-1)
