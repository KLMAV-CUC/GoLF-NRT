...
class GoLFModel(object):
    def __init__(self, args, load_opt=True, load_scheduler=True):
        ...
        self.global_encoder = GLNT_E().to(device)
        self.global_decoder = GLNT_D().to(device)
        ...
        learnable_params += list(self.global_encoder.parameters())
        learnable_params += list(self.global_decoder.parameters())
        ...
        if self.net_fine is not None:
            ...
            self.optimizer = torch.optim.Adam(
                [
                    {"params": self.net_coarse.parameters()},
                    {"params": self.global_encoder.parameters()},
                    {"params": self.global_decoder.parameters()},
                    {"params": self.feature_net.parameters(), "lr": args.lrate_feature},
                ],
                lr=args.lrate_gnt,
            )
    ...
    def save_model(self, filename):
        to_save = {
            ...
            "global_encoder": de_parallel(self.global_encoder).state_dict(),
            "global_decoder": de_parallel(self.global_decoder).state_dict(),
        }
