...
class GNT(nn.Module):
    ...
    def forward(self, rgb_feat, ray_diff, mask, pts, ray_d, glob_feat, query_feat):
        ...
        # q_init -> maxpool
        if query_feat == None:
            q_global = glob_feat.unsqueeze(1).unsqueeze(1).repeat(1,rgb_feat.shape[1],rgb_feat.shape[2],1)
            q_feat = torch.cat((q_global, rgb_feat), dim=2)
            q = q_feat.max(dim=2)[0]
        else:
            coarse_feat = query_feat
            q = query_feat.unsqueeze(1).repeat(1,rgb_feat.shape[1],1)
        ...
        if self.ret_alpha:
            return torch.cat([outputs, attn], dim=1), h
        else:
            return outputs
