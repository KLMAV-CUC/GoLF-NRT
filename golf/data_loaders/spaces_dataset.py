...
class SpacesFreeDataset(Dataset):
    def __init__(self, args, mode, **kwargs):
        self.folder_path = "../../../hdd/u202220081001015/data/spaces_dataset/data/800/"
        self.mode = mode
        self.num_source_views = args.num_source_views
        ...
        train_scene_ids = [i for i in np.arange(0, 5) if i not in eval_scene_ids]
        if mode == "train":
            self.scene_dirs = [
                os.path.join(self.folder_path, "scene_{:03d}".format(i)) for i in train_scene_ids
            ]
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train" and self.random_crop:
            render_rgb, render_camera, ref_rgbs_np, ref_cameras = random_crop(
                render_rgb, render_camera, ref_rgbs_np, ref_cameras, size=(384, 720)
            )
