...
sys.path.append("../")
from .data_utils import rectify_inplane_rotation, get_nearest_pose_ids, random_crop, center_pad
...
class NerfSyntheticDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.folder_path = "../../../hdd/u202220081001015/data/nerf_synthetic/"
    ...
    def __getitem__(self, idx):
        ...
        render_intrinsics_o = self.render_intrinsics[idx]
        ...
        rgb_o = rgb[..., [-1]] * rgb[..., :3] + 1 - rgb[..., [-1]]

        # resize
        rgb = cv2.resize(rgb_o, (720, 768), interpolation=cv2.INTER_AREA)
        mask = rgb[..., [-1]]
        render_intrinsics = render_intrinsics_o.copy()
        render_intrinsics[0,:] = render_intrinsics_o[0,:] * 720/800
        render_intrinsics[1,:] = render_intrinsics_o[1,:] * 768/800
        ...
        for id in nearest_pose_ids:
            ...
            src_rgb_o = src_rgb[..., [-1]] * src_rgb[..., :3] + 1 - src_rgb[..., [-1]]
            train_pose = train_poses[id]
            train_intrinsics_o = train_intrinsics[id]

            # resize
            src_rgb = cv2.resize(src_rgb_o, (720, 768), interpolation=cv2.INTER_AREA)
            train_intrinsics_ = train_intrinsics_o.copy()
            train_intrinsics_[0,:] = train_intrinsics_o[0,:] * 720/800
            train_intrinsics_[1,:] = train_intrinsics_o[1,:] * 768/800
        ...
        # rgb, camera, src_rgbs, src_cameras = random_crop(rgb, camera, src_rgbs, src_cameras, size=(768,720))
